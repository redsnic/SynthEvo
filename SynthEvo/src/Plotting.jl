...
function quick_trajectory_plot(C, p0, input0, input1, t0, t1, target, u0=nothing, prec=10)
    ...
    for i in 1:length(solutions)
        sol = merge_solutions(shared_solution, solutions, i, t0)
        if i == 1
...
function quick_sensitivity_plot(C, p0, input0, input1, t0, t1, target, u0=nothing, prec=10)
    ...
    """
    if u0 == nothing
        u0 = [0 for _ in 1:C.N+length(C.sensitivity_variables)]
    end
    base_problem = make_base_problem(C, C.ext_ode, u0, [input0], p0, t0, true, 1/prec)
    shared_solution, solutions = run_SF_CPU(C, base_problem, t0, t1, input1, 1e-5, 1e-5, true, 0.25)
...
function quick_IO_plot(C, p0, input0, input1, t0, t1, tf, target, u0=nothing, prec=10)
    """
    Quickly plot a trajectory of the CRN.
    ...
    - `p0`: the parameters
    - `input0`: the initial input
    - `input1`: the final input (a list)
    - `t0`: the initial time
    - `t1`: the final time
    - `tf`: time at which to check the IO relationship
    - `prec`: the precision of the plot
    ...
    for i in 1:length(solutions)
        sol = merge_solutions(shared_solution, solutions, i, t0)
        push!(output, sol(tf)[target])
    end
    plot(input1, output, title="Input vs Output", xlabel="Input", ylabel="Output", label=false)
    return plot!()
end
